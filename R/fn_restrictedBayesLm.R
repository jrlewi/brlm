...
#'
#' Fits the linear regression model \eqn{\beta~N(\mu_0, \Sigma_0)}, \eqn{\sigma^2~IG(a0, b0)},
#' \eqn{y~N(X\beta, \sigma^2 I)}
#'
#' Uses Gibbs sampling to sample from the restricted posterior under the above linear regression
#' model conditioned on estimators \code{regEst} and \code{scaleEst} for \eqn{\beta} and
#' \eqn{\sigma}, respectively. This is just the Gibbs sampler for the full posterior (implemented in
#' \code{brlm::bayesLm} augmented with a step to sample new data given all of the parameters and the
#' observed conditioning statistics
#'
#'
#' @inheritParams bayesLm
#' @param regEst The estimator of \eqn{\beta}  to condition on. Current options available are
#'   \code{Huber} and \code{Tukey} for Huber's and Tukey's (bisquare) estimators, respectively.
#' @param scaleEst The estimator of \eqn{\sigma} to condition on. Currently Huber's proposal 2 is
#'   the only option available. Specified with \code{Huber}.
#' @param maxit maximum number of iterations for use in \code{\link[MASS]{rlm}} when computing
#'   conditioning statistic during each proposal within the MCMC step for [y | rest, observed
#'   statistics]
#' @return list with mcmc sample, mean fitted values, robust regression object from \code{rlm}, and
#'   observed conditioning statistics.
#' @export
restrictedBayesLm <-
  function(y,
