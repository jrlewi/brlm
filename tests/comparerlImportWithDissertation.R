...
#
#
# tst<-brlm::rlImportSamp(X=X,y=y, psi=psi.bisquare, scale.est=scale.est, mu0=mu0, Sigma0=Sigma0, alpha=alpha, beta=beta, smooth=1,N=1e4,Nins=1e4, maxit=10000)
#
# plot(tst$w, cex=.1, pch=19)
# plot(density(tst$impSamps[,1],weights=tst$w))
# abline(v=coef(tst$fit)[1], col=2)
# plot(density(tst$impSamps[,2],weights=tst$w))
# abline(v=coef(tst$fit)[2], col=2)
# plot(density(tst$impSamps[,3],weights=tst$w))
# abline(v=summary(tst$fit)$sigma, col=2)
# abline(h=11)
