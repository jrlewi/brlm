...
#'@param fn.psi custom psi function returning psi(x) or psi'(x) defining M-estimator for location ;
#'  default is fn.psi.huber
#'@param fn.chi custom chi function returning chi(x) or chi'(x) defining M-estimator for scale;
#'  default (and currently only capability) is fn.chi.prop2
...
#'@return list y.curr: the new (if accepted) or same (if rejected) data vector, a: indicator of
#'  acceptance of proposed value 1 if accepted, 0 if rejected, log.prop.den.curr: proposal density
#'  on log scale of the returned data vector, rat: MH ratio, log.rat1: log ratio of proposal
#'  densities
#'@export
fn.one.rep.y <-
  function(y.curr,
           ...
           nu = NULL)
  {
    ...
    log.rat1 <-
      log.fn.eval.lik(y.prop, X, Beta, sig, base_model = base_model, nu = nu) -
      log.fn.eval.lik(y.curr, X, Beta, sig, base_model = base_model, nu = nu)

    log.prop.den.prop <-
      log.prop.den(y.prop, X, proj, l1obs, s1obs, fn.psi, fn.chi, n, p)
    ...
    if (tmp < rat) {
      y.curr <- y.prop
      a <- 1
      log.prop.den.curr <- log.prop.den.prop
    } else {
      a <- 0
    }
...
#'@details Two equivalent versions, the second uses \code{fn.attenuation2} and is faster. This
#'  function is designed for use within an MCMC sampler. Many of the inputs are iteration dependent.
#'  The input \code{log.prop.den.curr} is designed as a time saver. Since this is free of other
#'  parameters it can be saved from iteration to iteration to save computation time. Note the use of
#'  both fn.psi (custom in the \link{brlm} package) and psi (from the \link{MASS} package). This is
#'  poor coding and should be corrected.
#'@export
fn.one.rep.y2 <-
  function(y.curr,
           Beta,
           ...
           nu = NULL)
  {
    ...
    log.prop.den.prop <-
      log.prop.den2(y.prop, X, proj, Qt, l1obs, s1obs, fn.psi, fn.chi, n, p)
